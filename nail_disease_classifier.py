...
print("Loading Nail Disease Dataset...")
print(f"Expected folder structure: {DATA_DIR}/[{', '.join(NAIL_CONDITIONS)}]/")
...
plt.suptitle('Sample Nail Images from Training Set', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig('sample_nail_images.png', dpi=300, bbox_inches='tight')
print("✓ Sample images saved as 'sample_nail_images.png'\n")



def build_custom_cnn(num_classes):
    """
    Custom CNN architecture for nail disease classification
    """
...
def build_resnet_model(num_classes):
    ...
    (Excellent for medical imaging with fine details)
    """
...
def build_efficientnet_model(num_classes):
    """
    Transfer learning model using EfficientNetB2
    ...
    model = keras.Model(inputs, outputs, name='EfficientNetB2_Transfer')
...
if num_misclassified > 0:
    ...
    if 'Healthy' in class_names and 'Melanoma' in class_names:
        healthy_idx = class_names.index('Healthy')
        melanoma_idx = class_names.index('Melanoma')
        
        healthy_as_melanoma = np.sum((y_true == healthy_idx) & (y_pred == melanoma_idx))
        melanoma_as_healthy = np.sum((y_true == melanoma_idx) & (y_pred == healthy_idx))
        
        print(f"\n⚠️  CRITICAL Misclassifications:")
        print(f"  Healthy predicted as Melanoma: {healthy_as_melanoma} times")
        print(f"  Melanoma predicted as Healthy: {melanoma_as_healthy} times")
...
model.save('nail_disease_classifier_final.keras')
print("✓ Final model saved as 'nail_disease_classifier_final.keras'")
...
print("✓ TFLite model saved as 'nail_disease_model.tflite'")
...
with open('evaluation_report.txt', 'w') as f:
    f.write("="*60 + "\n")
    f.write("NAIL DISEASE CLASSIFIER - EVALUATION REPORT\n")
    f.write("="*60 + "\n\n")
...
def predict_single_nail(image_path, model, class_names):
    """
    Predict nail condition from a single image
    """
    ...
    print(f"\n{'='*60}")
    print(f"NAIL CONDITION PREDICTION: {os.path.basename(image_path)}")
    print(f"{'='*60}")
    print(f"\nPrimary Diagnosis: {class_names[predicted_idx]}")
    print(f"Confidence: {confidence:.2%}\n")
    
    if predicted_idx == class_names.index('Melanoma') if 'Melanoma' in class_names else -1:
        print("⚠️  WARNING: Melanoma detected! Immediate medical consultation recommended!")
    
    print(f"Top {num_top} Predictions:")
...
def predict_batch_nails(image_paths, model, class_names):
    """
    Predict multiple nail images
    """
    results = []
    for img_path in image_paths:
        condition, conf, probs = predict_single_nail(img_path, model, class_names)
        results.append({
            'image': img_path,
...
print("="*60)
print("  ✓ nail_disease_classifier_final.keras  - Trained model")
print("  ✓ nail_disease_model.tflite            - Mobile model")
print("  ✓ best_nail_disease_model.keras        - Best checkpoint")
...
print("  ✓ sample_nail_images.png               - Sample images")
print("  ✓ evaluation_report.txt                - Detailed metrics")

print("\n" + "="*60)
print("NEXT STEPS")
print("="*60)
print("1. Review the confusion matrix - especially Melanoma vs Healthy")
print("2. Use the prediction tools to test on new images:")
print("   predict_single_nail('path/to/nail.jpg', model, class_names)")
print("3. Pay special attention to Melanoma false negatives")
print("4. Consider ensemble models for critical diagnoses")

print("\n" + "="*60)
print("CLINICAL REMINDER")
print("="*60)
print("⚠️  This model is for screening purposes only!")
print("   All positive Melanoma predictions should be verified by a dermatologist.")
print("   Melanoma is life-threatening - early detection saves lives.")
print("="*60 + "\n")
